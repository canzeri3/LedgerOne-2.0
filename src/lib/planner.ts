...
/** Helpers for stable, tick-aware comparisons (prevents false "off-plan") */
...
function inferTickFromLevels(levels: BuyLevel[]): number {
  // Infer the tightest price precision present among level prices (up to 8 dp)
...
/**
 * BUY WATERFALL (USD-based) — price eligibility + lock-in
 * Strict mode remains: tolerance defaults to 0 (buy above a level never counts).
 * Enhancement: we round both trade & level prices to a detected (or provided) tick,
 * and allow a tiny absolute epsilon (~2 ticks). This prevents "off-plan" when the
 * user buys exactly at the target but floating math or formatting introduces dust.
 *
 * @param levels
 * @param trades
 * @param tolerance   fraction (e.g., 0.0035 for 0.35%). Default 0.0 (strict).
 * @param opts.tick   optional explicit tick size (e.g., 0.0001 for DOT). If omitted,
 *                    we infer from levels' price precision.
 * @param opts.absTickEps optional absolute epsilon override. Defaults to 2 * tick.
 */
export function computeBuyFills(
  levels: BuyLevel[],
  trades: BuyTrade[],
  tolerance: number = 0.0,
  opts?: { tick?: number; absTickEps?: number }
): BuyFillResult {
  ...
  // Price precision handling (tick + small absolute epsilon)
  const inferredTick = opts?.tick ?? inferTickFromLevels(levels) // e.g., 0.01 for $7.65, 0.0001 for many alts
  const absEps = opts?.absTickEps ?? (inferredTick > 0 ? 2 * inferredTick : 0)
  ...
  for (const b of byTime) {
    ...
    // Tick-round the trade price once
    const Pr = inferredTick > 0 ? roundToTick(P, inferredTick) : P

    // deepest eligible index inside price-sorted order:
    // eligible if Pr <= Lr * (1 + tolerance) OR within a tiny absolute epsilon
    let deepestK = -1
    for (let k = 0; k < byPriceDesc.length; k++) {
      const idx = byPriceDesc[k]
      const L = levels[idx]
      const Lp = Number(L.price)
      const Lr = inferredTick > 0 ? roundToTick(Lp, inferredTick) : Lp

      const withinPct = Pr <= Lr * (1 + tolerance)
      const withinAbs = absEps > 0 && Math.abs(Pr - Lr) <= absEps

      if (withinPct || withinAbs) deepestK = k
    }
